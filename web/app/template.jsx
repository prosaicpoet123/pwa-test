...
import {checkIfOffline, unlockScroll, finishedRetryingConnection} from './containers/app/actions'
...
const initPage = (component) => (url, routeName) => (dispatch, getState) => {
    ...
    return fetchResolve
        .then(() => {
            window.Progressive.initialRenderComplete &&
                window.Progressive.initialRenderComplete(getState())
        })
        .then(() => {
        ...
        .catch((error) => {
            ...
            window.Progressive.initialRenderFailed &&
                window.Progressive.initialRenderFailed(error)
        })
